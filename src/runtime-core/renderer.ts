...
function baseCreateRenderer(options) {
  ...
  const patchKeyChildren = (c1, c2, el) => {
    ...
    // abde
    let i = 0
    let e1 = c1.length - 1 // 老孩子中最后一项的索引
    let e2 = c2.length - 1 // 新孩子中最后一项的索引
    while (i <= e1 && i <= e2) {
      const n1 = c1[i]
      const n2 = c2[i]
    ...
    if (i > e1) {
      ...
      // 无规律的情况 diff算法
      // ab [cde] fg  // s1=2 e1=4
      // ab [edch] fg // s2=2 e2=5 => [5,4,3,0] 无视它
      const s1 = i
      const s2 = i
      // 新的索引 和 key做成一个映射表
      ...
      // 最长增长序列[0,1] [0,1,2,3]
      let increasingIndexSeqrence = getSequence(newIndexToOldMapIndex)
      let j = increasingIndexSeqrence.length - 1
      for (let i = toBePatched - 1; i >= 0; i--) {
        ...
        if (newIndexToOldMapIndex[i] == 0) {
          ...
          // 根据参照物 将节点直接移动过去 所有几点都要移动 （但是有些节点可以不动）
          // 此时没有考虑不动的情况
          // hostInsert(nextChild.el, el, anchor)
          // 优化后的 最长递增子序列
          if (j < 0 || i != increasingIndexSeqrence[j]) {
            hostInsert(nextChild.el, el, anchor)
          } else {
            j--
          }
  ...
  // getSequence([2,5,4,8,11,6]) getSequence([4,5,3,0])
  // 2
  // 2 5
  // 2 4 8
  // 2 4 6 11 => 4
